...
export async function deploySite(directory) {
	...
	const sourceBranch =
		process.env.CURRENT_BRANCH ?? shell.exec("git rev-parse --abbrev-ref HEAD", { silent: true }).stdout.trim();
	const organizationName = process.env.ORGANIZATION_NAME ?? siteConfig.organizationName;
	if (!organizationName) {
		throw new Error(
			`Missing project organization name. Did you forget to define "organizationName" in config? You may also export it via the ORGANIZATION_NAME environment variable.`,
		);
	}
	const projectName = process.env.PROJECT_NAME ?? siteConfig.projectName;
	if (!projectName) {
		throw new Error(
			`Missing project name. Did you forget to define "projectName" in config? You may also export it via the PROJECT_NAME environment variable.`,
		);
	}
  ...
  You can also set the deploymentBranch property in docusaurus.config.js .`);
	...
	const runDeploy = async () => {
		const toPath = await fs.mkdtemp(path.join(os.tmpdir(), `${projectName}-${deploymentBranch}`));
		shell.cd(toPath);
		if (
			shell.exec(`git clone --depth 1 --branch ${deploymentBranch} ${deploymentRepoURL} "${toPath}"`).code !== 0
		) {
			shell.exec(`git clone --depth 1 ${deploymentRepoURL} "${toPath}"`);
			shell.exec(`git checkout -b ${deploymentBranch}`);
		}
		shell.exec(`git rm -rf ${directory}`);
		const targetPath = path.join(toPath, path.basename(directory));
		console.log(72, toPath, directory, targetPath);
