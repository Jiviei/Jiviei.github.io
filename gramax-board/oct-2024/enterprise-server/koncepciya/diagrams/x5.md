...
workspace {
    ...
    model {
        ...
        desktopEditor -> gramaxEnterpriseService "Авторизируется чтобы зайти в воркспейс организации. Клонирует, пулит и пушит репозитории" "HTTPS"

        docPortal -> fileSystem "Хранит каталоги" "OS API"
        docPortal -> gramaxEnterpriseService "Перенаправляет пользователя для входа по SSO. Клонирует и пулит репозитории" "HTTPS"
