...
export const buildStaticSite = async (sourceDir, targetDir) => {
    ...
    await copyDir(path.join(__dirname, "/client"), targetDir);
    await copyCatalogDir(sourceDir, path.join(targetDir, path.basename(sourceDir)));
    const templateHtml = await fs.readFile(path.join(__dirname, "/client/index.html"), "utf-8");
    ...
    const rendered = await (await import(pathToFileURL(path.join(__dirname, "/server/server.js")).href)).default(catalogName);
