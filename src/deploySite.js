...
const loadConfig = (configPath) => {
    ...
    try {
        console.log(`${LogLevel.INFO} Reading the configuration file from "${configPath}"...`);
        ...
        if (error instanceof Error) {
            if (error.message.includes("YAMLSemanticError")) {
                ...
                console.error(`${LogLevel.ERROR} Error reading the configuration file: ${error.message}`);
            }
        }
        else {
...
export const deploySite = async (directory) => {
    ...
    const deploy = async () => {
        ...
        try {
            ...
            console.log(`${LogLevel.INFO} Commiting changes...`);
            ...
            if (commitResults.code === 0) {
                ...
                throw new Error(`Commit failed: ${commitResults}`);
            }
